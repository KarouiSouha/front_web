...
// ✅ Props depuis AgingReceivablePage
// - snapshotCount : combien de derniers snapshots afficher (depuis le filtre Period)
// - branch        : branche sélectionnée (affichage uniquement — les snapshots
//                   n'ont pas de dimension branche, donc pas de filtre possible)
interface AgingHistoricalTrendProps {
  snapshotCount?: number;   // undefined = tous les snapshots
  branch?:        string;   // pour afficher un avertissement
}
...
function CustomTooltip({ active, payload, label }: any) {
  ...
  const metricRow = (lbl: string, val: string, sub: string, color: string) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: '1px solid #f1f5f9' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 11, color: C.slateLight }}>{lbl}</span>
      </div>
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '14px 16px', boxShadow: '0 10px 30px rgba(0,0,0,0.12)', fontSize: 12, minWidth: 280 }}>
      <p style={{ fontWeight: 700, color: C.slate, marginBottom: 10, fontSize: 12, letterSpacing: '0.03em', textTransform: 'uppercase' }}>{label}</p>
      ...
      <p style={{ fontSize: 10, fontWeight: 600, color: C.slateXLight, textTransform: 'uppercase', letterSpacing: '0.07em', margin: '10px 0 6px' }}>Amounts</p>
...
// ─── Main component ───────────────────────────────────────────────────────────
export function AgingHistoricalTrend({ snapshotCount, branch }: AgingHistoricalTrendProps) {
  const [allData, setAllData] = useState<SnapshotTrend[]>([]);
  const [loading, setLoading] = useState(true);
  const [error,   setError]   = useState<string | null>(null);
  const [mode,    setMode]    = useState<'customers' | 'amount'>('customers');

  // ✅ Fetch TOUS les snapshots une seule fois — le filtre "period" s'applique
  // en slicant les N derniers snapshots (côté client), car les snapshots aging
  // sont indexés par numéro d'import et non par date de transaction.
  ...
  // ✅ Filtre Period appliqué ici : garde les N derniers snapshots.
  // snapshotCount=undefined → tous les snapshots.
  const data = snapshotCount ? allData.slice(-snapshotCount) : allData;
  ...
  if (error || allData.length === 0) return (
    <div style={{ ...card, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 200, gap: 8, color: C.mutedFg, fontSize: 13 }}>
      <span>{error ?? 'No data — import multiple snapshots to view trends.'}</span>
    </div>
  );

  if (data.length === 0) return (
    <div style={{ ...card, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 200, gap: 8, color: C.mutedFg, fontSize: 13 }}>
      <span>No snapshots match the selected period.</span>
      <span style={{ fontSize: 11, color: C.slateXLight }}>Try selecting "All Periods" to see the full history.</span>
    </div>
  );
  ...
  const kpis = [
    { label: 'Paid Customers',    value: `${latest.paid_pct}%`,       sub: `${latest.paid_customers} of ${latest.total_customers}`,      color: C.teal,    muted: C.tealMuted,    bdr: C.tealBorder,    delta: prev ? +(latest.paid_pct       - prev.paid_pct).toFixed(1)       : null, good: true  },
    { label: 'Overdue Customers', value: `${latest.overdue_pct}%`,    sub: `${latest.overdue_customers} of ${latest.total_customers}`,    color: C.crimson, muted: C.crimsonMuted, bdr: C.crimsonBorder, delta: prev ? +(latest.overdue_pct    - prev.overdue_pct).toFixed(1)    : null, good: false },
    { label: 'Overdue > 60 Days', value: `${latest.overdue60_pct}%`,  sub: `${latest.overdue60_customers} of ${latest.total_customers}`,  color: C.amber,   muted: C.amberMuted,   bdr: C.amberBorder,   delta: prev ? +(latest.overdue60_pct  - prev.overdue60_pct).toFixed(1)  : null, good: false },
    { label: 'Current Amount',    value: `${latest.collected_rate}%`, sub: fmt(latest.current_amount),                                   color: C.teal,    muted: C.tealMuted,    bdr: C.tealBorder,    delta: prev ? +(latest.collected_rate - prev.collected_rate).toFixed(1) : null, good: true  },
    { label: 'Overdue Amount',    value: `${latest.overdue_rate}%`,   sub: fmt(latest.overdue_amount),                                   color: C.crimson, muted: C.crimsonMuted, bdr: C.crimsonBorder, delta: prev ? +(latest.overdue_rate   - prev.overdue_rate).toFixed(1)   : null, good: false },
  ];
  ...
  return (
    <div style={card}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: C.mutedFg, margin: '0 0 0 13px' }}>
            ...
            {/* ✅ Affiche combien de snapshots sont visibles vs total */}
            <span style={{ marginLeft: 8, fontSize: 11, color: snapshotCount ? C.indigo : C.slateXLight, fontWeight: snapshotCount ? 600 : 400 }}>
              · {data.length} of {allData.length} snapshot{allData.length !== 1 ? 's' : ''}
              {snapshotCount ? ` (last ${snapshotCount})` : ' (all)'}
            </span>
          </p>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {/* ✅ Badge branche — explique clairement pourquoi pas filtrable */}
          {branch && (
            <div style={{
              display: 'flex', alignItems: 'center', gap: 5,
              padding: '5px 10px', borderRadius: 8,
              background: C.amberMuted, border: `1px solid ${C.amberBorder}`,
              ...
              Company-wide · no branch filter
            </div>
          )}

          {/* Toggle */}
          <div style={{ display: 'flex', borderRadius: 8, border: `1px solid ${C.border}`, overflow: 'hidden', background: C.bg }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10, marginBottom: 24 }}>
        {kpis.map((kpi, i) => (
          <div key={i} style={{ padding: '14px 16px', borderRadius: 10, background: kpi.muted, border: `1px solid ${kpi.bdr}`, position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: kpi.color, opacity: 0.6 }} />
            <p style={{ fontSize: 10, fontWeight: 600, color: C.slateXLight, margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{kpi.label}</p>
            <p style={{ fontSize: 22, fontWeight: 800, color: kpi.color, margin: 0, letterSpacing: '-0.02em', lineHeight: 1 }}>{kpi.value}</p>
            <p style={{ fontSize: 11, color: C.slateLight, margin: '4px 0 0' }}>{kpi.sub}</p>
            {kpi.delta !== null && (
              <p style={{ fontSize: 10, margin: '6px 0 0', fontWeight: 600, color: (kpi.good ? kpi.delta >= 0 : kpi.delta <= 0) ? C.teal : C.crimson }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead>
            <tr>
              {['Snapshot', 'Customers', 'Paid', 'Overdue', 'Overdue > 60d', 'Current Amount', 'Overdue Amount', 'Total'].map(h => (
                <th key={h} style={{ padding: '8px 12px', textAlign: 'left', color: C.slateXLight, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', fontSize: 10, whiteSpace: 'nowrap', borderBottom: `2px solid ${C.border}` }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {[...data].reverse().map((row, i) => (
              <tr key={row.snapshot_id} style={{ borderBottom: `1px solid ${C.border}`, background: i === 0 ? '#f0f4ff' : 'transparent', transition: 'background 0.1s' }}>
